...
# =========================================================================
# Matplotlib 서버 환경 한글 깨짐 방지 기본 설정
# =========================================================================
...
def load_assets():
    ...
    # 공백 제거 및 대문자 변환으로 기입 실수 방지
    data.columns = data.columns.str.strip()
    
    # 💡 영어 이름이든 한글 이름이든 무조건 한글로 통일시키는 안전 장치
    rename_dict = {
        'DATE_TIME': '발생일시', 'date_time': '발생일시',
...
# -------------------------------------------------------------------------
# 2. 데이터 전처리 (KeyError 방지 안전 예외처리 추가)
# -------------------------------------------------------------------------
# 혹시나 이름 변환이 실패했을 경우를 대비해 실제 존재하는 컬럼명을 동적으로 찾습니다.
target_date_col = '발생일시' if '발생일시' in df.columns else 'DATE_TIME'
target_dc_col = '직류전력량' if '직류전력량' in df.columns else 'DC_POWER'

# 날짜 변환 및 시간 변환 수행
df['변환일시'] = pd.to_datetime(df[target_date_col], errors='coerce')
...
with tab1:
    ...
    with col1:
        ...
        # 데이터에 존재하는 열만 안전하게 추출
        available_cols = [c for c in ['시간', '직류전력량', '교류전력량', '당일발전량', 'DC_POWER', 'AC_POWER', 'DAILY_YIELD'] if c in df.columns]
    ...
    with col2:
        ...
        df_hourgroup = df.groupby(df['변환일시'].dt.hour)[target_dc_col].mean()
    ...
    optime = df[df[target_dc_col] > 0]['시간']
    if not optime.empty:
        c1, c2 = st.columns(2)
